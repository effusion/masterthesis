...
Die Prototypen wurden bis anhin ohne sehr einfach gehalten. Für den Proof of Concept war es nötig gewisse Vorarbeiten zu erledigen. Dafür für die Hilfe eines OpenShift Administrators in Anspruch genommen um erste Teile zu erledigen.\newline
...
Zweites brauchte es ein neues Docker Base Image welches auf Red Hat aufbaut, Java Installiert und die passenden Berechtigungen gesetzt hat. Das Image wurde direkt auf der OpenShift Plattform gebaut, da dort das Software Repository von Red Hat angehängt ist, und anschliessend in das Firmen Interne Image Repository gelegt.\newline
Damit die Daten auch gespeichert werden können, wurde das Template 'MongoDB Persistent' mit allen vorgegebenen Parametern konfiguriert und installiert.
Der Spring Boot Prototyp für MongoDB wurde mit einem zusätzlichen REST Controller erweitert, welche die Spring Cloud Config Annotation hat, die Verbindung zur MongoDB konfiguriert und ausgerollt . Die Anbindung an den Config Server wurde noch weg gelassen da dieser noch nicht umgestellt wurde.
In einem nächsten Schritt musste der Config Server so angepasst werden, dass er die Daten vom GIT Repository lesen kann. Dafür mussten der SSH Key für den Zugriff auf den Server und die 'Known-hosts'\footnote{Datei welche die SSH Keys von anderen Systeme enthält welchen vertraut wird.} Datei im Docker Image abgelegt und zusätzlich die SshSessionFactory angepasst werden. Eine Lösung des Problems fand sich in \cite{sccssh}. Als Base Image wurde das gleiche verwendet wie für die Spring Boot Applikation.
Mit dem Config Server in Betrieb konnten die ersten Einstellung im Repository hinterlegt und die Spring Boot Applikation mit dem Server konfiguriert werden. Der letzte fehlende Teil war die Message Queue, welche die am Config Server angehängten Applikationen bei einer Änderung benachrichtigt. Auf OpenShift war für RabbitMQ kein Template vorhanden weshalb ein eigenes gemacht werden musste. Auch eine Internet Suche ergab für Red Hat kein Dockerfile welches als Basis hätte verwendet werden können. Die Suche wurden dann auf CentOS\footnote{Ein Linux Variante welche auf dem Source Red Hat Enterprise Linux basiert und deshalb weitgehend kompatible ist.} erweitert. Die gefunden Vorlage aus \cite{rmcentos7} musste noch kurz angepasst werden, sprich das Base Image wurde ausgetauscht und anschliessend wieder auf OpenShift gebaut. Schlussendlich wurde die Konfiguration für die Spring Boot Applikation mit den neuen Einstellung für die RabbitMQ erweitert und installiert.
